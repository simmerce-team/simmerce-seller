...
import { AccountInfoStep, BusinessInfoStep, AddressInfoStep } from './components';
  ...
  useEffect(() => {
    const fetchBusinessTypes = async () => {
      ...
      try {
        ...
        toast.error('Failed to load business types. Please refresh the page or try again later.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 3) {
      ...
      try {
        ...
        toast(error instanceof Error ? error.message : 'An error occurred during signup');
      } finally {
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {step === steps.length ? 'Creating account...' : 'Processing...'}
                  </>
                ) : step === steps.length ? (
                  'Create account'
                ) : (
